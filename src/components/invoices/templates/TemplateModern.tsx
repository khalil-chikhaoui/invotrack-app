...
const getLogoDimensions = (size: string) => {
  switch (size) {
    ...
    case "Large": return { width: 250, height: 125 };
    default: return { width: 150, height: 75 };
  }
};

const createStyles = (primaryColor: string, accentColor: string, secondaryColor: string) => {
  const PAGE_TOP_PADDING = 40;
  return StyleSheet.create({
    page: { fontFamily: "Cairo", fontSize: 10, lineHeight: 1.4, color: "#333", paddingTop: PAGE_TOP_PADDING, paddingBottom: 65 },
    headerBlock: { backgroundColor: primaryColor, marginTop: -2 * PAGE_TOP_PADDING, padding: `${PAGE_TOP_PADDING + 20} 30 30 30`, flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", color: "#FFFFFF" },
    headerLeft: { width: "55%", alignItems: "flex-start" },
    headerRight: { width: "45%", alignItems: "flex-end", justifyContent: "center" },
    businessName: { fontWeight: 700, color: "#FFF", marginBottom: 4, textTransform: "uppercase" },
    businessText: { fontSize: 9, color: "#FFF", opacity: 0.9, fontWeight: 300 },
    invoiceNumber: { fontSize: 24, fontWeight: 900, color: "#FFF", marginTop: -10, marginBottom: 5 },
    infoSection: { padding: "20 30", flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#F3F4F6" },
    sectionLabel: { fontSize: 9, fontWeight: 700, color: secondaryColor, textTransform: "uppercase", marginBottom: 5 },
    tableContainer: { padding: "0 30" },
    tableHeader: { flexDirection: "row", backgroundColor: "#F9FAFB", paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#E5E7EB" },
    th: { fontSize: 9, fontWeight: 700, color: primaryColor, textTransform: "uppercase" },
    ...
    bottomContentContainer: { flexDirection: "row", justifyContent: "space-between", padding: "20 30 0 30", marginTop: 10 },
    ...
    qrFooterLeft: { position: "absolute", bottom: 20, left: 30 },
  });
};

export default function TemplateModern({ invoice, business, settings, t, locale }: InvoiceTemplateProps) {
  ...
  const styles = useMemo(() => createStyles(primary, accent, secondary), [settings.color]);
  const logoDim = getLogoDimensions(settings.logoSize);
  const format = (amt: number) => formatMoney(amt, business.currency, business.currencyFormat);

  // Helper for Date Translation
  const renderDate = (dateString: string) => {
    if (!dateString) return "";
    const date = new Date(dateString);
    const formatStr = business.language === "en" ? "MMMM dd, yyyy" : "dd MMMM yyyy";
    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.headerBlock}>
        ...
        <View style={styles.headerRight}>
          ...
          <View style={{ marginTop: 12, alignItems: "flex-end" }}>
            <Text style={{ fontSize: 9, opacity: 0.8, fontWeight: 400, color: "#FFF", textTransform: "uppercase", marginTop: 10 }}>{t("issueDate")}</Text>
            <Text style={{ fontSize: 10, fontWeight: 700, color: "#FFF", marginBottom: 6 }}>{renderDate(invoice.issueDate || invoice.createdAt)}</Text>
            {settings.visibility.showDueDate && invoice.dueDate && (
              <>
                <Text style={{ fontSize: 9, opacity: 0.8, fontWeight: 400, color: "#FFF", textTransform: "uppercase" }}>{t("dueDate")}</Text>
                <Text style={{ fontSize: 10, fontWeight: 700, color: "#FFF" }}>{renderDate(invoice.dueDate)}</Text>
              </>
            )}
          </View>
        </View>
      ...
      <View style={styles.tableContainer}>
        ...
        {invoice.items.map((item, i) => (
          <View key={i} style={[styles.tableRow, { backgroundColor: i % 2 !== 0 ? hexToRgba(secondary, 0.08) : "transparent" }]} wrap={false}>
            <Text style={[styles.colDesc, { fontSize: 10, fontWeight: 500 }]}>{t(item.name)}</Text>
            <Text style={styles.colQty}>{item.quantity}</Text>
            <Text style={styles.colRate}>{format(item.price)}</Text>
            <Text style={[styles.colTotal, { fontWeight: 700 }]}>{format(item.total)}</Text>
          </View>
        ))}
      </View>
        ...
        <View style={styles.summarySection}>
          <View style={styles.summaryBox}>
            <View style={styles.summaryRow}>
              ...
              <Text style={{ fontWeight: 700 }}>{format(invoice.subTotal)}</Text>
            </View>
            {settings.visibility.showDiscount && invoice.totalDiscount > 0 && (
              <View style={styles.summaryRow}>
                <Text style={{ color: accent }}>{t("discount")}</Text>
                <Text style={{ fontWeight: 700, color: accent }}>-{format(invoice.totalDiscount)}</Text>
              </View>
            )}
            <View style={styles.summaryRow}>
              <Text style={{ fontWeight: 500 }}>{t("tax")} ({invoice.taxRate}%)</Text>
              <Text style={{ fontWeight: 700 }}>{format(invoice.totalTax)}</Text>
            </View>
            <View style={styles.grandTotalRow}>
              <Text style={{ fontWeight: 700, color: primary }}>{t("total")}</Text>
              <Text style={{ fontSize: 16, fontWeight: 900, color: primary }}>{format(invoice.grandTotal)}</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      {settings.visibility.showFooter && settings.footerNote && (
        <Text style={styles.fixedFooter} fixed>{settings.footerNote}</Text>
      )}
