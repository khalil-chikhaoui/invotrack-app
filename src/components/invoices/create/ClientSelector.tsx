...
import { HiOutlineMagnifyingGlass, HiOutlineUser, HiPlus, HiChevronDown, HiOutlineXMark } from "react-icons/hi2";
    ...
    return (
      <span>
        {parts.map((part, i) => 
          ...
          <b key={i} className="text-brand-600 dark:text-brand-400 font-semibold">{part}</b> : part
        )}
      </span>
    );
  };
      ...
      <div className="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.05] rounded-md shadow-sm flex flex-col transition-all">
        ...
        <div className="px-6 py-3 border-b border-gray-100 dark:border-white/5 flex items-center justify-between bg-white dark:bg-gray-900/50 rounded-t-md">
          ...
          {!isEdit && (
            ...
            >
              <HiPlus className="size-3.5" /> New Client
            </button>
          )}
        </div>
          ...
          {!selectedClient ? (
            <div className="relative group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <HiOutlineMagnifyingGlass className={`size-4 transition-colors ${searching ? 'text-brand-500' : 'text-gray-400'}`} />
              </div>
              <input
                type="text"
                value={search}
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                {searching ? (
                  ...
                  <HiChevronDown className={`size-4 text-gray-300 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
                )}
              </div>
              ...
              {isOpen && (
                <div className="absolute z-50 w-full mt-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl shadow-brand-500/10 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                  <div ref={scrollParentRef} className="max-h-[240px] overflow-y-auto custom-scrollbar">
                      ...
                      displayItems.map((c, index) => (
                        ...
                        >
                          <div className="relative w-9 h-9 rounded-lg bg-gray-100 dark:bg-white/5 flex items-center justify-center shrink-0 overflow-hidden border border-gray-200/50 dark:border-white/5">
                            {c.logo ? <img src={c.logo} className="w-full h-full object-cover" /> : <span className="text-xs font-semibold text-gray-400">{c.name.charAt(0)}</span>}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="text-sm font-medium text-gray-800 dark:text-white truncate uppercase tracking-tight leading-tight"><HighlightText text={c.name} highlight={search} /></span>
                            <span className="text-[10px] text-gray-600 dark:text-gray-300 truncate font-medium"><HighlightText text={c.email || 'No email'} highlight={search} /></span>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="p-6 text-center">
                        <p className="text-xs font-medium text-gray-600 dark:text-gray-300">No results found</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-white dark:bg-transparent border border-brand-500/20 shadow-sm animate-in zoom-in-95 duration-200">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center shrink-0 border border-brand-100 dark:border-brand-500/20 overflow-hidden">
                  {selectedClient.logo ? <img src={selectedClient.logo} className="w-full h-full object-cover" /> : <HiOutlineUser className="text-brand-500 size-5" />}
