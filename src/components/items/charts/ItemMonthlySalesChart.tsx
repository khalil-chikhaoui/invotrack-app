...
export default function ItemMonthlySalesChart({
  itemId,
  currency = "USD",
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const data = await invoiceApi.getItemMonthlySales(itemId, selectedYear);
        setSalesData(data.sales);
        if (data.years && data.years.length > 0) {
          setAvailableYears(data.years);
        }
      } catch (error) {
        console.error("Failed to load item monthly sales", error);
