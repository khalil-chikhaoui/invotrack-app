 ...
 * Provides a structured list of countries with ISO codes and Dial codes.
 */

export interface CountryData {
  name: string;
  code: string; // ISO 3166-1 alpha-2 (e.g., "DE", "TN")
  dial_code: string; // Phone prefix (e.g., "+49", "+216")
}

// 1. Define the Priority Codes (Order matters here)
const PRIORITY_CODES = ["FR", "DE", "TN", "US", "IT", "ES"];
...
const RAW_COUNTRIES: CountryData[] = [
  ...
  { name: "Tunisia", code: "TN", dial_code: "+216" }, // Priority
  ...
  { name: "United States", code: "US", dial_code: "+1" }, // Priority
...
// 3. Logic to create the final sorted list
// This puts your priority countries first, then the rest alphabetically.
const PRIORITY_COUNTRIES = PRIORITY_CODES.map((code) =>
  RAW_COUNTRIES.find((c) => c.code === code),
).filter((c): c is CountryData => c !== undefined);

const OTHER_COUNTRIES = RAW_COUNTRIES.filter(
  (c) => !PRIORITY_CODES.includes(c.code),
).sort((a, b) => a.name.localeCompare(b.name));

// 4. Export the combined list
// Note: We removed the "---" string separator because it breaks the Object type safety.
// If you need a visual separator, it's best handled in the UI component rendering.
