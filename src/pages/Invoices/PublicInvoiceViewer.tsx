...
export default function PublicInvoiceViewer() {
  ...
  const displayBusiness = useMemo(() => {
    ...
    const formattedStyle = styleParam
      ...
      : null;
    const isValidStyle = ["Classic", "Modern", "Minimal", "Receipt"].includes(
      formattedStyle || "",
    );
    ...
    return {
      ...
      invoiceSettings: {
        ...
        template:
          isValidStyle && formattedStyle
            ? formattedStyle
            : business.invoiceSettings?.template,
      },
    } as BusinessData;
  ...
  if (isMobile) {
    return (
      <BlobProvider
        document={<InvoicePDF invoice={invoice} business={displayBusiness} />}
      >
